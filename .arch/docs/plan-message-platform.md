...
**P13 (ops controls, snapshot, explorer)** — інтеграційні O01–O04 у Messaging.Tests (реальні PostGIS + RabbitMQ), O05 unit у Admin.Tests, O07 міграція в
Integration.Tests, A03/A04 Playwright:

```powershell
pwsh -File scripts/with-lock.ps1 dotnet test tests/Puluj.Messaging.Tests --filter "FullyQualifiedName~OpsControlTests"        # O01 pause scope, O02 drain, O03 snapshot/alarms, O04 lifecycle
...
| P06 | done | Claude Code (p06) | p06_review: approve after fixes (B1–B6, N1–N13, Q1–Q3) → виправлено → re-run 64/0/1 | [GitHub P06](https://github.com/sql-monk/Puluj-g/issues/8); [handoff, evidence](evidence/message-platform/P06-handoff.md); llm-worker (lease/fencing/audit/deferral), finalizer (один extraction на run, observations/analysis events), topology v5; проміжний коміт `a9c7ec4` (owner), правки review — наступний коміт |
