...
або Docker/Testcontainers), `cd web && npm test` (ETA), `cd web && npm run e2e` — Playwright E2E/visual мапи й адмінки над замоканим API (P12; `npx playwright install chromium` один раз; знімки win32-only).
...
| `/api/admin/analytics/*` | `status` (курсор, відставання, прогони), `report?days=` (джерела, пари, зовнішні пересилання, перші по треках), `recent?limit=&sourceId=` (пари з текстами), `POST reset` (copy-аналітика; `lifecycle_*` стан не чіпає) |
| `/api/admin/analytics/lifecycle` (P15) | `GET ?hours=24\|168\|720` — звіт «Аналітика повідомлень» (`funnel` з окремими знаменниками raw/posts/analyzed, `timeline` (година / Kyiv-доба), `sources`, `parse`, `quality` (precision/recall `unavailable`), `cost`, `results`, `history`, `reconciliation`); `GET /status` (backfill cursor, останній звіт звірки); `POST /backfill?reset=` і `POST /reconcile?hours=` (`{actor, reason}`, audit `analytics:*`) ([ADR-0013](adr/ADR-0013-message-analytics.md)) |
| `POST /api/admin/dev/ingest` | вкинути повідомлення чи payload тривоги як від колектора (тестування) |
| `/api/admin/incidents` (P10) | `GET` (`state`, `kind`, `hours`, `limit`; P14: лише active generation, `generation=all` показує shadow з `generationActive`), `GET /{id}` (incident + links + revisions зі snapshot), `POST /{id}/resolve|retract|confirm|suppress|unsuppress` (`{actor, reason, effectiveAt?}`), `POST /merge` (`{sourceId, targetId, actor, reason}`), `POST /{id}/split` (`{observationIds[], actor, reason}`); усі команди — через `IncidentStateWriter` з `incident.changed` в outbox; 400 без actor/reason, 404, 409 за станом/kind/вимкненим outbox ([ADR-0010](adr/ADR-0010-incidents.md)) |
| `/api/admin/event-kinds` (P12) | catalog editor: `GET` (усі kinds + `iconVocabulary`/`renderModes`), `PUT /{code}` (`{nameUk?, mapVisible?, mapColor?, mapIcon?, mapLifetime?, renderMode?, sortOrder?, enabled?, requiresLocationForMap?, actor, reason, force?}` — лише presentation, admin-owned після зміни; 400 без actor/reason, 404, 409 `enabled=false` для legacy-mapped kind без `force`, 422 невалідні значення), `GET /{code}/audit` ([ADR-0008](adr/ADR-0008-event-catalog.md)) |
| `/api/admin/incidents/review?hours=&limit=`, `GET /{id}/merge-preview?targetId=` (P12) | черга ревʼю (один рядок на incident, links `ambiguous`/`near_candidates`, кандидати з поточним станом і `mergeable`, `truncated` при переповненні сторінки), preview злиття без запису (ті самі правила, що `POST /merge`; `targetRevision` для stale-guard); `GET /{id}` тепер з `segmentText`/`rawText`/`rawUrl` per link (admin-only) |
| `/api/admin/ops/messaging` (P13) | snapshot шини зі квитанцій БД: `subscriptions[]` (subscription×lane: pending/inFlight/retry/quarantined/oldest age/event-time lag/p50-p95-p99/completed за годину/консюмери; `ready/unacked` з management API, коли задано `Messaging:Broker:ManagementUrl`, `source: db\|management`), `roots`, `broker`, `outbox`, `inbox`, `reconciliation`, `workers[]` (+`consumers[]`, `stale`/`stuck`), `backfill[]`, `alarms[]` (`{code, severity, scope, message}`), `slo`; `?fresh=true` минає кеш 5 с. `GET /quarantine?subscription&open&limit`, `GET /audit?limit`; `POST /lanes/{subscription}/{lane}` (`{state active\|paused\|draining, actor, reason}`), `POST /quarantine/{id}/retry\|waive` (`{actor, reason}`), `POST /scale` (`{service processor\|messaging, replicas, actor, reason}`; 409 поза Docker); 400 без actor/reason ([ADR-0012](adr/ADR-0012-ops-controls.md)) |
| `/api/admin/messages?q&sourceId&hours&limit`, `GET /{rawId}/lifecycle` (P13) | message explorer: пошук raw (id / ключ джерела / текст ILIKE ≤ 168 год, `limit ≤ 100`), картка lifecycle — події (≤ 200) з усіма deliveries і attempts (помилки повністю), extractions/observations, похідні (tracks/alerts/incidents), quarantine, `summary {completion, waiting[], completed[], failed[]}`; url лише `http(s)` |
| `/api/admin/ops/runs` (P14) | runs/generations: `GET ?limit=` (`RunDto[]` — стан, generation + `generationActive`, scope з `verification`, checkpoint), `POST /replay` (`{from, to, sourceIds?, actor, reason}` → run `created`, 409 якщо є відкритий replay run), `POST /{id}/start\|pause\|resume\|cancel\|catchup\|verify\|promote\|rollback` (`{actor, reason, watermark?}`; `promote?force=true` для partial scope; 400/404/409 з причиною state machine) ([ADR-0005](adr/ADR-0005-runs-completion.md) «Orchestration») |
