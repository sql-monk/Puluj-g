...
| [0005](ADR-0005-runs-completion.md) | Runs/generations/lanes, completion manifest, terminal outcomes, workflow status; P14: replay run state machine, checkpoint за PK + ingest ceiling, delta catchup, verify/promote/rollback (advisory lock active generation), shadow incident-worker, supersede live run | accepted (P14; receipts P03, stage_results P05, finalizer P06) | P01, P03, P05, P06, P14 | tracks/alerts generations, history `completed`, legacy reset cutover — P16; partial merge — після даних |
...
| [0013](ADR-0013-message-analytics.md) | Lifecycle projection `analytics.message_lifecycle` per (raw, run): consumer `message-analytics` (v10), знаменники raw/пости/редакції, backfill з evidence (legacy = unavailable), reconciliation пізніх результатів і лічильників, звіт «Аналітика повідомлень», copy-аналітика як допоміжний розділ | accepted (P15) | P15 | precision/recall, legacy retirement, daily-агрегати, `:e{ts}` reader — P16 |
| [0012](ADR-0012-ops-controls.md) | Ops metrics/health зі квитанцій БД (subscription×lane, roots, outbox/inbox, воркери+`Consumers[]`), management API як опційне джерело, чисті alarm-правила з SLO (`Ops:Slo`), runtime-контролі per lane (`subscription_lanes`: pause/resume/drain через consumer poll, `control_audit`), scale, message explorer (одна картка lifecycle) | accepted (P13) | P13 | DB pool/readiness/LLM budget/quorum — P16; backfill/replay-контролі — P14; silent source — P14/P15 |
