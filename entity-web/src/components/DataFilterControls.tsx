...
export default function DataFilterControls({ route }: { route: PublicRoute }) {
  ...
  useEffect(() => {
    let active = true
    Promise.all([api.taxonomy(), api.eventKinds(), api.sources(), api.regions(), entityApi.definitions()])
      .then(([taxonomy, eventKinds, sources, regions, definitions]) => active && setDictionary({ taxonomy, eventKinds, sources, regions, entityKinds: definitions.map((definition) => definition.entityName) }))
  ...
  const entitySection = route.section === 'entities'
  ...
  return <div className="flex flex-col gap-3 text-sm">
    ...
    <label>Тип сутності<select multiple className={box} value={filters.entityKinds} onChange={(event) => apply({ ...filters, entityKinds: strings(event), cursor: undefined })}>{dictionary.entityKinds.map((kind) => <option key={kind} value={kind}>{kind}</option>)}{unknownEntityKinds.map((kind) => <option key={kind} value={kind}>Недоступна сутність: {kind}</option>)}</select></label>
    {!entitySection && <><label>Вид події<select multiple className={box} value={filters.eventKinds} onChange={(event) => apply({ ...filters, eventKinds: strings(event), cursor: undefined })}>{dictionary.eventKinds.map((kind) => <option key={kind.code} value={kind.code}>{kind.nameUk} · {kind.code}</option>)}{unknownKinds.map((code) => <option key={code} value={code}>Недоступний historical code: {code}</option>)}</select></label>
