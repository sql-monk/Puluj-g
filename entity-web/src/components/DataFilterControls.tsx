  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([taxonomy, eventKinds, sources, regions, definitions]) => {
        if (!active) return
        setDictionary({ taxonomy, eventKinds, sources, regions, entityKinds: definitions.map((definition) => definition.entityName) })
