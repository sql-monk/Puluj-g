...
import { sortEntityItemsNewestFirst } from './detailsPresentation'
...
export default function EntityFeedPanel({ open, onToggle, items, route, loading, error, history = false }: {open:boolean; onToggle:()=>void; items:EntityItem[]; route:PublicRoute; loading:boolean; error?:string; history?:boolean}) {
  const ordered = sortEntityItemsNewestFirst(items)
  ...
  return <aside aria-label="Записи карти" data-map-occlusion="true" className={`absolute ${history ? "bottom-[calc(var(--replay-height,16rem)+1.5rem)]" : "bottom-24"} right-3 top-56 z-20 flex w-[calc(100%-1.5rem)] flex-col rounded-xl bg-white/95 shadow-lg sm:top-48 md:w-96 dark:bg-slate-900/95`}>
    ...
    <div className="overflow-y-auto p-3 text-sm"><p className="mb-2 text-xs text-slate-500">Новіші записи спочатку. Показано сутності поточного знімка, включно із записами без координат.</p>{loading && <p role="status">Оновлення…</p>}{error && <p role="alert" className="text-red-600">Не вдалося оновити дані. Показано попередній знімок.</p>}{!items.length && !loading && !error && <p>За цими фільтрами записів немає.</p>}
      <ul className="space-y-2">{ordered.map((item) => <li key={`${item.entity}:${item.id}`}><a className="block break-words rounded border p-2 hover:bg-slate-100 dark:hover:bg-slate-800" href={publicHash({section:'entities',detail:{kind:item.entity,id:item.id},query:route.query})}><span className="flex items-center gap-2"><EntityIcon entity={item.entity} values={item.values} /><b className="min-w-0 [overflow-wrap:anywhere]">{String(item.values.label ?? item.values.name ?? item.values.title ?? entityLabel(item.entity))}</b></span><span className="block text-xs text-slate-500">{entityLabel(item.entity)} · №{item.id}{item.occurredAt && ` · ${new Date(item.occurredAt).toLocaleString('uk-UA', {timeZone:'Europe/Kyiv'})}`}</span></a></li>)}</ul>
    </div>
  </aside>
}
