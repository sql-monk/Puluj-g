  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <StatTile label="Версій повідомлень" value={compact(data.messages)} exactValue={`${data.messages.toLocaleString('uk-UA')} версій за часом публікації`} note={`з ${data.sources.length} джерел`} />
        <StatTile label="Оброблено" value={pct(data.processed, data.messages)} exactValue={`${data.processed.toLocaleString('uk-UA')} з ${data.messages.toLocaleString('uk-UA')} версій`} note={`${compact(data.processed)} версій`} />
        <StatTile label="З розпізнаними фактами" value={pct(data.withTargets, data.processed)} exactValue={`${data.withTargets.toLocaleString('uk-UA')} з ${data.processed.toLocaleString('uk-UA')} версій`} note={`${compact(data.withTargets)} з оброблених`} />
      ...
      <ChartCard title="Джерела" subtitle="версії повідомлень за часом публікації; канонічні факти наведено окремо" empty={empty}>
        <SourcesTable sources={data.sources} />
      </ChartCard>

      <ChartCard
        title="Версії повідомлень за джерелами"
        subtitle={`версій ${perBucket(p.bucket)}, за часом публікації`}
        empty={empty}
        legend={series.map((s) => (
      ...
      >
        <Columns labels={labels} titles={titles} series={series} valueLabel="версій повідомлень" height={240} />
      </ChartCard>
    </>
  )
}
