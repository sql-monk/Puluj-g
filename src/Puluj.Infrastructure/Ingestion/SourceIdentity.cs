...
/// <summary>
/// Raw identity derived from `raw_messages.source_message_id`: a Telegram edit `"{id}:e{unix}"` is revision `e{unix}` of key `{id}`;
/// everything else (originals, alerts `"{id}:start|end"`, unknown sources) is revision `"0"` of the id itself.
/// collectors can also provide the key and revision explicitly.
/// </summary>
