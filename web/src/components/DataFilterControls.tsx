...
export default function DataFilterControls({ route }: { route: PublicRoute }) {
  ...
  return <div className="flex flex-col gap-3 text-sm">
    ...
    <label>Тип сутності<select multiple className={box} value={filters.entityKinds} onChange={(event) => apply({ ...filters, entityKinds: strings(event), cursor: undefined })}>{['track', 'alert', 'observation'].map((kind) => <option key={kind} value={kind}>{kind}</option>)}</select></label>
