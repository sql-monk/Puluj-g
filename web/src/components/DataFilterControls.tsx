...
export default function DataFilterControls({ route }: { route: PublicRoute }) {
  ...
  return <div className="flex flex-col gap-3 text-sm">
    ...
    <label>Тип сутності<select multiple className={box} value={filters.entityKinds} onChange={(event) => apply({ ...filters, entityKinds: strings(event), cursor: undefined })}>{['track', 'incident', 'alert', 'observation'].map((kind) => <option key={kind} value={kind}>{kind}</option>)}</select></label>
    <label>Вид події<select multiple className={box} value={filters.eventKinds} onChange={(event) => apply({ ...filters, eventKinds: strings(event), cursor: undefined })}>{dictionary.eventKinds.map((kind) => <option key={kind.code} value={kind.code}>{kind.nameUk} · {kind.code}</option>)}{unknownKinds.map((code) => <option key={code} value={code}>Недоступний historical code: {code}</option>)}</select></label>
    <label>Категорія події<select multiple className={box} value={filters.eventCategories} onChange={(event) => apply({ ...filters, eventCategories: strings(event), cursor: undefined })}>{['target', 'alert', 'incident', 'info'].map((category) => <option key={category} value={category}>{category}</option>)}</select></label>
...
function Chips({ filters, onChange }: { filters: DataQuery; onChange: (next: DataQuery) => void }) {
  const entries: [keyof DataQuery, string][] = [
    ['q', filters.q ? `Пошук: ${filters.q}` : ''], ['eventKinds', filters.eventKinds.length ? `Види: ${filters.eventKinds.join(', ')}` : ''], ['eventCategories', filters.eventCategories.length ? `Категорії: ${filters.eventCategories.join(', ')}` : ''], ['sourceIds', filters.sourceIds.length ? `Джерела: ${filters.sourceIds.join(', ')}` : ''], ['regionId', filters.regionId ? `Область #${filters.regionId}` : ''], ['from', filters.from && filters.to ? 'Період' : ''],
