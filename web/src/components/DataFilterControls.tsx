...
function Chips({ filters, onChange }: { filters: DataQuery; onChange: (next: DataQuery) => void }) {
  const entries: [keyof DataQuery, string][] = [
    ['q', filters.q ? `Пошук: ${filters.q}` : ''], ['eventKinds', filters.eventKinds.length ? `Види: ${filters.eventKinds.join(', ')}` : ''], ['eventCategories', filters.eventCategories.length ? `Категорії: ${filters.eventCategories.join(', ')}` : ''], ['sourceIds', filters.sourceIds.length ? `Джерела: ${filters.sourceIds.join(', ')}` : ''], ['regionId', filters.regionId ? `Область #${filters.regionId}` : ''], ['outcome', filters.outcome ? `Результат: ${filters.outcome}` : ''], ['from', filters.from && filters.to ? 'Період' : ''],
