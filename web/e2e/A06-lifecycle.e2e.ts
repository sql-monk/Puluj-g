 ...
 * source conversion remains a count of messages (not generated objects), p50/p95 are explained in plain language, and the
...
const report = {
  ...
  funnel: { raw: 1200, posts: 1100, stored: 1200, analyzed: 1150, withFacts: 640, withTargets: 180, withEvents: 530, withIncidents: 410, withTracks: 120, withAlerts: 500, domainCompleted: 600, visible: 410, stuckAnalysis: 3, stuckDomain: 0, unavailableTimings: 250, unavailableCompletion: 250, storedToAnalyzedP50Seconds: 4.2, storedToAnalyzedP95Seconds: 18, analyzedToDomainP50Seconds: 1.1, analyzedToDomainP95Seconds: 9 },
  timeline: [{ at: '2026-09-16T10:00:00Z', raw: 50, analyzed: 48, withFacts: 20, withTargets: 8, withEvents: 16, withIncidents: 14, domainCompleted: 19, failed: 2, noText: 5 }, { at: '2026-09-16T11:00:00Z', raw: 60, analyzed: 60, withFacts: 30, withTargets: 10, withEvents: 24, withIncidents: 20, domainCompleted: 30, failed: 0, noText: 7 }],
  sources: [{ sourceId: 1, code: 'tg_kpszsu', raw: 700, posts: 650, edits: 50, noText: 12, withPayload: 12, analyzed: 680, withFacts: 400, withTargets: 180, withEvents: 330, withIncidents: 310, withTracks: 120, withAlerts: 0, facts: 400, textLengthP50: 180, collectDelayP50Seconds: 25, collectDelayP95Seconds: 90, live: 690, history: 10, maxGapSeconds: 5400 }, { sourceId: 2, code: 'alerts_in_ua', raw: 500, posts: 500, edits: 0, noText: 500, withPayload: 500, analyzed: 470, withFacts: 240, withTargets: 0, withEvents: 200, withIncidents: 100, withTracks: 0, withAlerts: 500, facts: 500, textLengthP50: null, collectDelayP50Seconds: 3, collectDelayP95Seconds: 8, live: 500, history: 0, maxGapSeconds: 600 }],
...
const status = { available: true, backfill: { cursor: 99000, maxRawMessageId: 99000, rawRows: 1200, projectedRows: 1200, caughtUp: true, at: '2026-09-16T11:59:00Z' }, reconciliation: { at: '2026-09-16T11:59:30Z', windowHours: 48, rawRows: 2300, posts: 2100, edits: 200, projectedRaw: 2300, projectedPosts: 2100, missingRoots: 0, lateAnalyses: 3, lateCompletions: 12, pendingAnalysis: 1, pendingDomain: 4, unavailableTimings: 250, unavailableCompletion: 250 } }
...
test.describe('admin lifecycle analytics', () => {
  test('A06 lifecycle page: source conversions, throughput and plain-language timing definitions are visible; reconciliation controls stay gated', async ({ page }) => {
    ...
    await expect(funnel).toContainText('З цілями')
    await expect(funnel).toContainText('180')
    await expect(page.getByTestId('transitions')).toContainText('p50 — половина повідомлень')
    await expect(page.getByTestId('transitions')).toContainText('Для 250 старих повідомлень етапи не записувалися')
    await expect(page.getByTestId('timeline')).toContainText('з подіями')
    // Conversion cells use a message denominator: source A has 180 messages with targets out of 700 received.
    await expect(page.getByTestId('source-tg_kpszsu')).toContainText('25,7 %')
    ...
    await expect(st).toContainText(/1[\s  ]200\s*\/\s*1[\s  ]200/)
    await expect(st).toContainText('проєкційовано / усі повідомлення')
